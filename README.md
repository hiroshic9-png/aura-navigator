...
- 👨‍⚕️ 1,469名の医師情報（名寄せ済み・LLM強化済み、専門医資格535名）
...
- 🛡️ 7つの法的行動支援ツール（クーリングオフ・カルテ開示・契約チェック他）
- 🧠 AIアドバイザー法規制RAG統合（厚生労働省改正・特商法・個人情報保護法）
...
| GET | `/api/advisor/tools` | 法的行動支援ツール一覧 |
| POST | `/api/advisor/tools/{id}` | ツール実行（テンプレート生成） |

## 法的行動支援ツール

美容医療の患者さんが「自分で行動を起こせる」テンプレートとチェックリスト。

| ツール | 説明 | 法的根拠 |
|--------|------|----------|
| 📝 クーリングオフ通知書 | 契約日から8日以内の無条件解約テンプレート | 特定商取引法第48条 |
| 📋 カルテ開示請求書 | 診療録の開示請求テンプレート | 個人情報保護法第33条 |
| ✅ 契約書チェックリスト | サイン前の12ポイント確認 | — |
| 💰 見積もり価格チェック | DB相場との比較 | — |
| 🩹 術後経過チェックリスト | 正常/異常の見分け方 | — |
| 🏥 クリニック比較シート | 4軸評価比較 | — |
| 📞 消費者センター相談準備 | 188ホットライン用準備シート | — |
...
- **AI**: Anthropic Claude API（ストリーミング対応・法規制RAG統合）
